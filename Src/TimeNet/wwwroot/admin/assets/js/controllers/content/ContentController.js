adminApp.controller('ContentController',
    function (ContentService, ReportService, $scope, $timeout, $http) {
        let content_table = null,
            getContentsUrl = '/admin/content/get'
        $scope.deleteids_arr = []
            ...
            content_table = $('#content_table').DataTable({
                ...
                "ajax": {
                    ...
                    beforeSend: function () {
                        loading.show()
                        if (!firstLoading) {
                            loadSkeletonTable('content_table')
                        }
                    },
                    complete: function () {
                        loading.hide()
                    ...
                    error: function () {
                        message.error()
                        $(`#content_table tbody tr`).empty()
                        $('#content_table_processing').hide()
                    },
                },
                columnDefs: [
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        render: function (col, type, row) {
                            let html = `<a href="/admin/content/update/${row.id}" class="table-row-key">${row.title}</a>`
                            console.log(row)
                            return html;
                        }
                    },
        ...
        $scope.deleteContents = function () {
            ContentService.BulkDelete($scope.deleteids_arr).then(function (data) {
                if (data) {
        ...
        $scope.selectedAll = function () {
            if ($scope.isSelectedAll) {
                $scope.deleteids_arr = []
                $('#content_table input[type="checkbox"]').prop('checked', true);
                $('#content_table input[type="checkbox"].delete_check:checked').each(function () {
                    $scope.deleteids_arr.push($(this).val());
                });
            }
            else {
                $scope.deleteids_arr = []
                $('#content_table input[type="checkbox"]').prop('checked', false);
            }
            $scope.updateTableUi()
        }
        ...
        $scope.export = async function () {
            const parms = content_table.ajax.params()
            ...
            const fileName = language === 'vi' ? 'thuong_hieu.xlsx' : 'contents_data_export.xlsx';
