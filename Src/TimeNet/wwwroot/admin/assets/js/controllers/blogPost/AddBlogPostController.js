adminApp.controller('AddBlogPostController',
    function (BlogPostService, StorageService, $scope, $timeout) {
        ...
        $scope.addBlogPost = async function (continueEdit) {
            if ($scope.addBlogPostform.$valid) {
                loading.show()
                ...
                if (files.length > 0) {
                    ...
                    if (fileUploaded)
                        $scope.blogPost.image = fileUploaded
                }
                ...
                BlogPostService.AddBlogPost($scope.blogPost).then(function (data) {
                    if (data) {
