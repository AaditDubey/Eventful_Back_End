adminApp.controller('BlogPostController',
    function (BlogPostService, ReportService, $scope, $timeout, $http) {
        let blogPost_table = null,
            getBlogPostsUrl = '/admin/blogPost/get'
        $scope.deleteids_arr = []
        ...
        $timeout(function () {
            blogPost_table = $('#blogPost_table').DataTable({
                ...
                "ajax": {
                    ...
                    beforeSend: function () {
                        loading.show()
                        if (!firstLoading) {
                            loadSkeletonTable('blogPost_table')
                        }
                    },
                    complete: function () {
                        loading.hide()
                    ...
                    error: function () {
                        message.error()
                        $(`#blogPost_table tbody tr`).empty()
                        $('#blogPost_table_processing').hide()
                    },
                },
                columnDefs: [
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        render: function (col, type, row) {
                            let html = `<a href="/admin/blogPost/update/${row.id}" class="table-row-key">${row.title}</a>`
                            console.log(row)
                            return html;
                        }
                    },
        ...
        $scope.getBlogPosts = function () {
            blogPost_table.ajax.reload();
        }
        $scope.setActive = function (val) {
            $scope.FilterModel.Published = val
            $scope.getBlogPosts();
        }
        $scope.deleteBlogPosts = function () {
            BlogPostService.BulkDelete($scope.deleteids_arr).then(function (data) {
                if (data) {
        ...
        $scope.selectedAll = function () {
            if ($scope.isSelectedAll) {
                $scope.deleteids_arr = []
                $('#blogPost_table input[type="checkbox"]').prop('checked', true);
                $('#blogPost_table input[type="checkbox"].delete_check:checked').each(function () {
                    $scope.deleteids_arr.push($(this).val());
                });
            }
            else {
                $scope.deleteids_arr = []
                $('#blogPost_table input[type="checkbox"]').prop('checked', false);
            }
            $scope.updateTableUi()
        }

        $scope.deleteSelected = function () {
            $scope.deleteBlogPosts()
        }
        ...
        $scope.export = async function () {
            const parms = blogPost_table.ajax.params()
            ...
            let url = `/admin/reports/ExportlogPostReport?published=${published}&pageIndex=${pageIndex}&pageSize=${pageSize}&language=${language}`
            if (searchText)
            ...
            const fileName = language === 'vi' ? 'thuong_hieu.xlsx' : 'blogPosts_data_export.xlsx';
