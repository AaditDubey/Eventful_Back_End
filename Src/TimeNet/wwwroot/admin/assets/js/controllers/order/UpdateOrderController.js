adminApp.controller('UpdateOrderController',
    function (OrderService, UserService, ProductService, $scope, $timeout) {
        ...
        $scope.orderStatuses = globalLocalizer.sales.orderStatuses
        ...
        $scope.GetOrder = function (id) {
            OrderService.GetOrder(id).then(function (data) {
                if (data) {
                    $scope.order = data;
                    $scope.order.customer = {
                        id: $scope.order.customerId,
                        email: $scope.order.customerEmail,
                        firstName: $scope.order.firstName,
                        lastName: $scope.order.lastName
                    }
                    $scope.order.lockEdit = $scope.order.orderStatus === 'Complete' || $scope.order.orderStatus === 'Cancelled'
                }
            });
        }

        $scope.updateOrder = async function (continueEdit) {
            if ($scope.updateOrderform.$valid) {
                loading.show()
                ...
                const order = await OrderService.UpdateOrder($scope.order)
                if (order) {
