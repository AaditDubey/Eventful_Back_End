adminApp.controller('AddUserController',
    function (UserService, RoleService, VendorService, $scope) {
        ...
        $scope.addUser = async function (continueEdit) {
            if ($scope.addUserform.$valid) {
                loading.show()
                const user = await UserService.AddUser($scope.user)
                if (user) {
                    const link = continueEdit ? `/admin/user/update/${data.id}` : `/admin/user`
