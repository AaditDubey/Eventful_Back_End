adminApp.controller('AddSpeakerController',
    function (SpeakerService, StorageService, $scope) {
        ...
        $scope.addSpeaker = async function (continueEdit) {
            if ($scope.addSpeakerform.$valid) {
                loading.show()
                ...
                if (files.length > 0) {
                    ...
                    if (fileUploaded)
                        $scope.speaker.image = fileUploaded
                 
                }
                SpeakerService.AddSpeaker($scope.speaker).then(function (data) {
                    if (data) {
