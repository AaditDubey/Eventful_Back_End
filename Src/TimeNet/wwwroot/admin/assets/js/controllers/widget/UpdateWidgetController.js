adminApp.controller('UpdateWidgetController',
    function (WidgetService, StorageService, BrandService, CategoryService, $scope, $timeout) {
        ...
        $scope.GetWidget = function (id) {
            WidgetService.GetWidget(id).then(function (data) {
                if (data) {
                    $scope.widget = data;
                    $scope.onWidgetTypeChange()
                    if ($scope.widget.type === 'HeaderMenus') {
                        $scope.widget.widgetMenus.forEach(menu => {
                            $scope.menuList.push({...menu, id: menu.menuId})
                        })
        ...
        $scope.updateWidget = async function (continueEdit) {
            if ($scope.updateWidgetform.$valid) {
                loading.show()
                if ($scope.widget.type === 'HeaderMenus') {
                ...
                const result = await WidgetService.Update($scope.widget)
                if (result)
